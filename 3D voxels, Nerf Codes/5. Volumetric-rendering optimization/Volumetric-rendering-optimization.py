...
class Sphere():
    ...
    def intersect(self, x): # intersect with ray and Sphere
        """
        :param x: points [batch_size, 3]
        """

        # (x- xc)^2 + (y-yc)^2 + (z-zc)^2 <= r^2
        # self.p = tensor([ 0,  0, -1]) , x = Tensor:(16000000, 3)
        cond = (x[:, 0] - self.p[0]) ** 2 + (x[:, 1] - self.p[1]) ** 2 + (x[:, 2] - self.p[2]) ** 2 <= self.r ** 2 # get the condition.
        # cond is 160000000 : [False, False, ...True...False] # each layer has the condition : 1505174
        num_rays = x.shape[0] # 16000000
...
def rendering(model, rays_o, rays_d, tn, tf, nb_bins=100, device='cpu'):
    ...
    alpha = 1 - torch.exp(- density * delta.unsqueeze(0)) # alpha is zero towards center, if density is high alpha is low  # [nb_rays, nb_bins, 1]
...
b_reshaped = b.reshape(H,W,3).cpu().data.numpy()
